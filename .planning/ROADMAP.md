...
Hard criteria (drastically reduced from the old 21-phase scope per ADR-0011):

1. **Signed release builds** install and run on developer's Android + iPhone (Phase 6 + 7)
2. **1-hour GPS pocket-session** records ≥95% of expected points on both platforms (Phase 7)
3. **Tag → distribute** works end-to-end: `git tag v1.0.0-beta.X` → CI publishes APK to Caddy + IPA to TestFlight internal group (Phase 8)
4. **5-10 friend testers** complete onboarding + 1 GPS session each within 72h of invite (Phase 9)
5. **Zero P0 crashes** in 72h watchlist; P1/P2 logged for v1.0.x patch cycle

That's the gate. No 48h soak. No 8-device matrix. No restore drill. No on-call rotation. Solo dev, friend testers, ship-when-stable.
...
*Milestone: v1.0 Closed Beta*
*Status: Phase 6 ready to plan via `/gsd-discuss-phase 6`*
