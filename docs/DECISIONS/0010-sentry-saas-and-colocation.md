...
- **R-08 NEW (2026-05-19 PM, post-Plan-05-02-Task-2 exposure event)** — Telegram bot token + chat.id leaked into Claude Code conversation transcript during Plan 05-02 v3 Task 2 (BotFather setup). User pasted BotFather's confirmation message verbatim (including `8099702979:AAHkpfVN6...`) and subsequent `getUpdates` JSON (including `chat.id: 8791445158`) into chat instead of writing them to a local scratchpad. User was offered rotation; chose **accept** for closed-beta scale. **Exploitable surface:** anyone with read-access to the conversation transcript can use the bot token to POST messages to chat.id `8791445158` (private DM to user `itITocka`) impersonating the alerts bot. Cannot read messages, cannot escalate to anything else. Cannot harm the user except via social engineering (fake "production is down" alerts at 3am → user could be phished). **Likelihood:** LOW for closed-beta — requires attacker watching this specific conversation. **Impact:** MEDIUM if attacker is patient + targeted. **Mitigation:** Rotation MANDATORY before v1.0-rc.1 → public-beta transition (R-08 closure gate). Add as explicit acceptance criterion to Phase 21 staging soak: "TELEGRAM_BOT_TOKEN rotated via BotFather `/revoke` since the leak; `getUpdates` returns 401 on the old token". Same rotation playbook applies to any future leak (one-liner: `/revoke` → pick bot → save new token → `sops edit .secrets/prod/sentry.yaml` → `make deploy`).
