...
Полные ротационные плейбуки (10 секрет-типов) приедут в Plan 02-04 — здесь
только скелет журнала per Plan 02-03 / D-12.

| Дата       | Секрет                   | Событие                                                                                       | Действия                                                                                                  | Статус             | Ссылка                                              |
|------------|--------------------------|-----------------------------------------------------------------------------------------------|-----------------------------------------------------------------------------------------------------------|--------------------|-----------------------------------------------------|
| 2026-05-06 | Все 5 Mapbox-токенов     | Pre-v1.0 chat leak: все токены однажды передавались в чат с AI-ассистентом                    | Treated-as-compromise full reset (запланирован в Plan 02-04 / SEC-03..04 + ротация по playbook ниже)      | In-progress (02-04)| (ADR появится в 02-04)                              |
| 2026-05-16 | gitleaks history scan    | Полное `--log-opts="--all"` сканирование репо (160 commits, 5.55 MB, gitleaks v8.30.1)        | Audit-артефакт зафиксирован в `docs/gitleaks-history-scan.json`                                           | Clean — 0 findings | [`docs/gitleaks-history-scan.json`](./gitleaks-history-scan.json) |
| 2026-05-16 | trufflehog history scan  | Verified-secret deep scan (2610 chunks, 5.80 MB, trufflehog v3.95.3, `--only-verified`)       | Audit-артефакт зафиксирован в `docs/trufflehog-history-scan.json`                                         | Clean — 0 findings | [`docs/trufflehog-history-scan.json`](./trufflehog-history-scan.json) |

**Вывод first-scan (2026-05-16):** в git-истории нет реальных `sk.`/`pk.` Mapbox-токенов
ни в одном commit'е. Единственный известный канал утечки — чат с AI-ассистентом
(документировано в Phase 0); митигируется ротацией всех 5 токенов в Plan 02-04 Task 4.
